...
for iVideo=1:nsessions
    ...
    if do_plot
        ...
        subplot(8,4,12)
        nPC=1;
        MaxPC1=[max(scores(:,nPC)) 0];
        PCA2frame(coeffs(:,1:2),MaxPC1,meanFrame,p)
        subplot(8,4,11)
        MinPC1=[min(scores(:,nPC)) 0];
        PCA2frame(coeffs(:,1:2),MinPC1,meanFrame,p)
        %% PC 2
        subplot(8,4,3)
        nPC=2;
        MaxPC2=[0 max(scores(:,nPC))];
        PCA2frame(coeffs(:,1:2),MaxPC2,meanFrame,p)
        subplot(8,4,7)
    ...
    PClength_all=[PClength_all;PClength];
    behaviour_all=[behaviour_all;behaviour];
    VarExp(:,iVideo)=VarexpTmp(1:20);
    counter=counter+1;
...
%% Dimensionality reduction B (left)
subplot(4,4,2)
plot(cumsum(VarExp),'Color',[0.5 0.5 0.5])
xlabel('N PC')
hold on
plot(cumsum(mean(VarExp,2,'omitnan')),'k','LineWidth',2)
